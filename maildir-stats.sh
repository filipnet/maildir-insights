...
TABLE="HOMEDIR|SIZE|COUNT|DIR|READ|UNREAD|TRASH\n"
for i in "${USERDIR[@]}"
do
	:
	...
	MAILBOX_DCOUNT=`find $i/Maildir -type d | wc -l`
	...
	TABLE+="${MAILBOX_MAILDIR}|${MAILBOX_SIZE}|${MAILBOX_COUNT}|${MAILBOX_DCOUNT}|${MAILBOX_READ}|${MAILBOX_UNREAD}|${MAILBOX_TRASH}\n"
done
